...
# Fonction pour la réplication effective des données
def replicate_data(data):
    # Simulation d'un léger délai pour la réplication asynchrone
    time.sleep(0.5)  # Léger délai de 0.5 seconde (modifiable selon les besoins)
    
    # Écriture des données sur le stockage secondaire
    with open('secondary_storage.json', 'w', encoding='utf-8') as file:
        json.dump(data, file, indent=4)
        
# Fonction pour la réplication asynchrone
def asynchronous_replication(data):
    # Utilisation d'un thread pour exécuter la fonction de réplication de manière asynchrone
    replication_thread = threading.Thread(target=replicate_data, args=(data,))
...
def add_product():
    ...
    asynchronous_replication(products)
...
def create_order():
    ...
    asynchronous_replication(orders)
...
def add_to_cart(userId):
    ...
    asynchronous_replication(carts)
